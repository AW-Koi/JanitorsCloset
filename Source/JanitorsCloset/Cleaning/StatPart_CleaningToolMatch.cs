...
namespace JanitorsCloset.Cleaning
{
    ...
    public class StatPart_CleaningToolMatch : StatPart
    {
        ...
        {
            var penalty = ResolveOffsetToSuppress(req);
        ...
        private static float ResolveOffsetToSuppress(StatRequest req)
        {
            ...
            if (toolExt.Matches(category.Value)) return 0f; // Match: keep the bonus.

            // Mismatch: subtract the weapon's own equipped CleaningSpeed offset.
            return EquippedCleaningSpeedOffset(tool.def);
        }
